...
const BtechSubjectDetailsScreen = ({ route, navigation, colors = { background: '#F8F9FE', surface: '#FFF', text: '#111827', textSecondary: '#6B7280', primary: '#1976D2' } }) => {
  const insets = useSafeAreaInsets();
  const { btechSubject } = route.params || {};
  const subjectId = btechSubject?.id;

  const dispatch = useDispatch();
  const { btechQuestions, loadingBtechQuestions, subjectPracticeProgress, bookmarks } = useSelector((state) => state.study);
  ...
  useEffect(() => {
    if (subjectId) {
      dispatch(fetchBtechQuestions(subjectId));
    ...
    const loadPracticeItems = async () => {
      ...
      try {
        const [mcqSnap, flashSnap] = await Promise.all([
          firestore().collection('btech_interview').where('subjectId', '==', subjectId).get(),
          firestore().collection('btech_flashcard').where('subjectId', '==', subjectId).get(),
        ]);
        ...
        console.error('Error loading B.Tech practice collections:', error);
      } finally {
  ...
  const renderQuestions = () => {
    const qList = btechQuestions[subjectId] || [];
    if (loadingBtechQuestions && qList.length === 0) {
      return (
          ...
          return (
            <View style={[styles.questionCardList, { backgroundColor: colors.surface }]}>
              <View style={styles.cardHeader}>
                ...
                <TouchableOpacity
                  onPress={() => dispatch(toggleBookmark({
                    questionItem: item,
                    questionType: 'btech',
                    sourceCollection: 'btechQuestions',
                    topicId: subjectId,
                    isBookmarked
                  }))}
                  style={{ padding: 4 }}
              ...
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('BtechAnswer', {
                    topicTitle: btechSubject.name,
                    questionItem: item,
                    topicId: subjectId,
                    colors
                  })
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>B.Tech</Text>
          <Text style={[styles.title, { color: colors.text }]} numberOfLines={1}>{btechSubject?.name}</Text>
        </View>
...
export default BtechSubjectDetailsScreen;
