...
import { fetchBtechSubjects } from '../../store/studySlice';

const BtechSubjectsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { btechSubjects, loadingBtechSubjects } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (true) {
      dispatch(fetchBtechSubjects());
    }
  }, [dispatch]);
  ...
  const renderSubject = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('BtechSubjectDetails', { btechSubject: item })}
      activeOpacity={0.8}
    >
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.subtitle}>B.Tech • Subjects</Text>
          <Text style={styles.title}>Select Subject</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>

      {loadingBtechSubjects ? (
        <View style={styles.center}>
          <ActivityIndicator size="large" color="#1976D2" />
        </View>
      ) : btechSubjects.length === 0 ? (
        <View style={styles.center}>
...
export default BtechSubjectsScreen;
