...
import { fetchDnsSubjects } from '../../store/studySlice';

const DnsSubjectsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { dnsSubjects, loadingDnsSubjects } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (true) {
      dispatch(fetchDnsSubjects());
    }
  }, [dispatch]);
  ...
  const renderSubject = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('DnsSubjectDetails', { dnsSubject: item })}
      activeOpacity={0.8}
    >
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.subtitle}>DNS • Subjects</Text>
...
export default DnsSubjectsScreen;
