...
const EtoSubjectDetailsScreen = ({ route, navigation, colors = { background: '#F8F9FE', surface: '#FFF', text: '#111827', textSecondary: '#6B7280', primary: '#1976D2' } }) => {
  const insets = useSafeAreaInsets();
  const { etoSubject } = route.params || {};
  const subjectId = etoSubject?.id;

  const dispatch = useDispatch();
  const { etoQuestions, loadingEtoQuestions, subjectPracticeProgress, bookmarks } = useSelector((state) => state.study);
  ...
  useEffect(() => {
    if (subjectId) {
      dispatch(fetchEtoQuestions(subjectId));
    ...
    const loadPracticeItems = async () => {
      ...
      try {
        const [mcqSnap, flashSnap] = await Promise.all([
          firestore().collection('eto_interview').where('subjectId', '==', subjectId).get(),
          firestore().collection('eto_flashcard').where('subjectId', '==', subjectId).get(),
        ]);
        ...
        console.error('Error loading ETO practice collections:', error);
      } finally {
  ...
  const renderQuestions = () => {
    const qList = etoQuestions[subjectId] || [];
    if (loadingEtoQuestions && qList.length === 0) {
      return (
          ...
          return (
            <View style={[styles.questionCardList, { backgroundColor: colors.surface }]}>
              ...
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('EtoAnswer', {
                    topicTitle: etoSubject.name,
                    questionItem: item,
                    topicId: subjectId,
                    colors
                  })
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>ETO</Text>
          <Text style={[styles.title, { color: colors.text }]} numberOfLines={1}>{etoSubject?.name}</Text>
        </View>
...
export default EtoSubjectDetailsScreen;
