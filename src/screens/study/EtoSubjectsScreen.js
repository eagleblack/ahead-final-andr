...
import { fetchEtoSubjects } from '../../store/studySlice';

const EtoSubjectsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { etoSubjects, loadingEtoSubjects } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (true) {
      dispatch(fetchEtoSubjects());
    }
  }, [dispatch]);
  ...
  const renderSubject = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('EtoSubjectDetails', { etoSubject: item })}
      activeOpacity={0.8}
    >
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.subtitle}>ETO • Subjects</Text>
...
export default EtoSubjectsScreen;
