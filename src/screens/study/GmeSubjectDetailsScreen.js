...
const GmeSubjectDetailsScreen = ({ route, navigation, colors = { background: '#F8F9FE', surface: '#FFF', text: '#111827', textSecondary: '#6B7280', primary: '#1976D2' } }) => {
  const insets = useSafeAreaInsets();
  const { gmeSubject } = route.params || {};
  const subjectId = gmeSubject?.id;

  const dispatch = useDispatch();
  const { gmeQuestions, loadingGmeQuestions, subjectPracticeProgress, bookmarks } = useSelector((state) => state.study);
  ...
  useEffect(() => {
    if (subjectId) {
      dispatch(fetchGmeQuestions(subjectId));
    ...
    const loadPracticeItems = async () => {
      ...
      try {
        const [mcqSnap, flashSnap] = await Promise.all([
          firestore().collection('gme_interview').where('subjectId', '==', subjectId).get(),
          firestore().collection('gme_flashcard').where('subjectId', '==', subjectId).get(),
        ]);
        ...
        console.error('Error loading GME practice collections:', error);
      } finally {
  ...
  const renderQuestions = () => {
    const qList = gmeQuestions[subjectId] || [];
    if (loadingGmeQuestions && qList.length === 0) {
      return (
          ...
          return (
            <View style={[styles.questionCardList, { backgroundColor: colors.surface }]}>
              ...
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() =>
                  navigation.navigate('GmeAnswer', {
                    topicTitle: gmeSubject.name,
                    questionItem: item,
                    topicId: subjectId,
                    colors
                  })
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>GME</Text>
          <Text style={[styles.title, { color: colors.text }]} numberOfLines={1}>{gmeSubject?.name}</Text>
        </View>
...
export default GmeSubjectDetailsScreen;
