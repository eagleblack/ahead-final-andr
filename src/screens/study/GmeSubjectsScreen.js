...
import { fetchGmeSubjects } from '../../store/studySlice';

const GmeSubjectsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { gmeSubjects, loadingGmeSubjects } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (true) {
      dispatch(fetchGmeSubjects());
    }
  }, [dispatch]);
  ...
  const renderSubject = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('GmeSubjectDetails', { gmeSubject: item })}
      activeOpacity={0.8}
    >
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.subtitle}>GME • Subjects</Text>
...
export default GmeSubjectsScreen;
