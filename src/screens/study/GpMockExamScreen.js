...
import { updateGpMockExamAnswer, updateGpMockExamIndex } from '../../store/studySlice';

const GpMockExamScreen = ({ route, navigation }) => {
  ...
  const handleOptionSelect = (index) => {
    dispatch(updateGpMockExamAnswer({
  ...
  const handleNext = () => {
    if (!isLastQuestion) {
      dispatch(updateGpMockExamIndex({
        subjectId: gpSubject.id,
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          text: "Submit", 
          onPress: () => {
            navigation.replace('GpMockExamResult', { gpSubject });
          }
        }
      ]
    );
...
export default GpMockExamScreen;
