...
import { fetchGpSubjects } from '../../store/studySlice';

const GpSubjectsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { gpSubjects, loadingGpSubjects } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (true) {
      dispatch(fetchGpSubjects());
    }
  }, [dispatch]);
  ...
  const renderSubject = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('GpSubjectDetails', { gpSubject: item })}
      activeOpacity={0.8}
    >
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.subtitle}>GP • Subjects</Text>
...
export default GpSubjectsScreen;
