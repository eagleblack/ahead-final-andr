...
import {
  clearStcwMockExamSession,
  fetchStcwMcqQuestions,
  createStcwMockExamSession,
} from '../../store/studySlice';

const StcwMockExamResultScreen = ({ route, navigation }) => {
  ...
  const handleFinish = () => {
    ...
    navigation.navigate('StcwSubjectDetails', {
      stcwLevel: {
        id: stcwSubject.stcwLevelId,
      },
    });
  };
  ...
  const handleRestart = async () => {
    ...
    try {
      ...
      const result = await dispatch(
        fetchStcwMcqQuestions(stcwSubject.id)
      );
      ...
      navigation.replace('StcwMockExam', {
        stcwSubject,
      });
    } catch (error) {
      console.error('Failed to restart STCW mock exam:', error);
    } finally {
      setLoading(false);
    }
  };
...
export default StcwMockExamResultScreen;
