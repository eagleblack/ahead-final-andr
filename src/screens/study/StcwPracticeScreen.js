...
import { fetchStcwPracticeQuestions, fetchBookmarks, toggleBookmark } from '../../store/studySlice';

const StcwPracticeScreen = ({ route, navigation, colors }) => {
  ...
  const { stcwPracticeQuestions, bookmarks, loadingStcwPracticeQuestions } = useSelector((state) => state.study);
  
  useEffect(() => {
    if (stcwSubject?.id) {
      dispatch(fetchStcwPracticeQuestions(stcwSubject.id));
    }
  }, [dispatch, stcwSubject?.id]);

  useEffect(() => {
    dispatch(fetchBookmarks());
  }, [dispatch]);

  const currentQuestions = stcwPracticeQuestions[stcwSubject?.id] || [];
  const isLoading = loadingStcwPracticeQuestions;
    ...
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={[styles.subtitle, { color: themeColors.textSecondary }]}>STCW • PRACTICE</Text>
            ...
            return (
              <View style={[styles.questionCard, { backgroundColor: themeColors.surface }]}>
                ...
                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() =>
                    navigation.navigate('StcwAnswer', {
                      topicTitle: stcwSubject.name,
                      questionItem: item,
                      topicId: stcwSubject.id,
                      colors: themeColors
                    })
...
export default StcwPracticeScreen;
