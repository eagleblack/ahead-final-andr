...
import { fetchStcwMcqQuestions, createStcwMockExamSession } from '../../store/studySlice';

const StcwSubjectDetailsScreen = ({ route, navigation }) => {
  const insets = useSafeAreaInsets();
  const dispatch = useDispatch();
  const { stcwLevel, stcwSubject } = route.params;
  const { stcwMockExam } = useSelector((state) => state.study);
  
  const activeSession = stcwMockExam?.sessions?.[stcwSubject.id];
  ...
  useEffect(() => {
    const checkQuestions = async () => {
      ...
      const result = await dispatch(fetchStcwMcqQuestions(stcwSubject.id));
      if (fetchStcwMcqQuestions.fulfilled.match(result)) {
    ...
    checkQuestions();
  }, [dispatch, stcwSubject.id, activeSession]);

  const startMockExam = async () => {
    if (activeSession) {
      navigation.navigate('StcwMockExam', { stcwSubject });
      return;
    }
    ...
    if (fetchStcwMcqQuestions.fulfilled.match(result)) {
      ...
      dispatch(createStcwMockExamSession({
    ...
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.title} numberOfLines={2}>{stcwSubject.name}</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.card]}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('StcwPractice', { stcwSubject })}
          >
...
export default StcwSubjectDetailsScreen;
