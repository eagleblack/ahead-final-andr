...
const StudyInterviewScreen = ({ navigation, colors }) => {
  ...
  const loadProgressAndQuestions = async (dept) => {
    try {
      ...
      // 2. Fetch MCQs (query-filter-free for absolute reliability)
      const mcqSnap = await firestore().collection('interview_mcqs').get();

      const mcqList = mcqSnap.docs
        .map((doc) => {
        ...
        .filter((item) => deptId ? item.departmentId === deptId : (!item.departmentId || item.departmentId === null || item.departmentId === undefined || item.departmentId === ''))
        .filter((item) => item.sequence > currentMcqSeq)
      ...
      // 3. Fetch FIBs (query-filter-free for absolute reliability)
      const fibSnap = await firestore().collection('interview_fill_blanks').get();

      const fibList = fibSnap.docs
        .map((doc) => {
          const rawData = doc.data();
          const qData = {};
          Object.keys(rawData).forEach((key) => {
        ...
        .filter((item) => deptId ? item.departmentId === deptId : (!item.departmentId || item.departmentId === null || item.departmentId === undefined || item.departmentId === ''))
        .filter((item) => item.sequence > currentFibSeq)
        .sort((a, b) => a.sequence - b.sequence)
        .slice(0, 20);
      ...
      // 4. Fetch Flashcards (query-filter-free for absolute reliability)
      const flashSnap = await firestore().collection('interview_flashcards').get();

      const flashList = flashSnap.docs
        .map((doc) => {
          const rawData = doc.data();
          const qData = {};
          Object.keys(rawData).forEach((key) => {
        ...
        .filter((item) => deptId ? item.departmentId === deptId : (!item.departmentId || item.departmentId === null || item.departmentId === undefined || item.departmentId === ''))
        .filter((item) => item.sequence > currentFlashSeq)
        .sort((a, b) => a.sequence - b.sequence)
        .slice(0, 20);
